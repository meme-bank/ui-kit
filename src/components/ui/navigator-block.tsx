        ...
        <div
            className={
                cn(
                    ...
                    (isActive ? "ms-bg-secondary ms-shadow" : "hover:ms-bg-secondary hover:ms-shadow"),
                    className
                )
            }
            onClick={action}
        >
            {Icon && <Icon className={cn("ms-h-8 ms-w-8 lg:ms-w-4 lg:ms-h-4", iconClassName)} />}
            {(IconRender && !Icon) && <IconRender />}
            {(!IconRender && !Icon) && <ChevronRightCircle className={cn("ms-h-8 ms-w-8 ms-rotate-90 lg:ms-w-4 lg:ms-h-4 lg:ms-rotate-0", iconClassName)} />}
            <Separator className={cn("ms-h-px ms-w-8 ms-duration-150 lg:ms-h-5 lg:ms-w-px", (isActive ? "mb-0" : "mb-1 lg:mb-0 group-hover:mb-0"))} />
            <div className={cn("ms-duration-150 ms-text-xs lg:ms-text-sm ms-text-ellipsis ms-block ms-overflow-hidden ms-whitespace-nowrap", (isActive ? "lg:ms-ml-1 ms-mb-1 lg:ms-mb-0" : "group-hover:lg:ms-ml-1 ms-mb-0 group-hover:ms-mb-1 group-hover:lg:ms-mb-0"))}>{children}</div>
        </div>
    )
};
Navigator.ButtonNoAdaptive = ({ Icon, children, action, className, isActive, IconRender, iconClassName }) => {
    return (
        ...
        >
            ...
            {(IconRender && !Icon) && <IconRender />}
            {(!IconRender && !Icon) && <ChevronRightCircle className={cn("ms-h-4 ms-w-4", iconClassName)} />}
            <Separator orientation="vertical" className="ms-h-5" />
            <div className={cn("ms-duration-150 ms-text-ellipsis ms-block ms-overflow-hidden ms-whitespace-nowrap", (isActive ? "ms-ml-1" : "group-hover:lg:ms-ml-1"))}>{children}</div>
        </div>
    )
};
...
Navigator.Header = ({ children }) => {
    ...
    return (
        <div className={cn("ms-flex ms-gap-2 ms-items-center ms-justify-between ms-duration-150", (!open ? "ms-mb-3" : "ms-mb-5"))}>
            <h4 className="ms-font-vksans">{children || "Навигация"}</h4>
            <div onClick={() => setOpen(state => !state)} className="ms-h-6 ms-w-6 ms-flex hover:ms-bg-accent hover:ms-text-accent-foreground ms-duration-150 ms-items-center ms-justify-center ms-cursor-pointer ms-rounded-full">
                <ChevronLeftCircle className={cn("ms-h-4 ms-w-4 ms-duration-150", !open && "ms-rotate-180")} />
            </div>
        </div>
    )
}
