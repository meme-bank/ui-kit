...
export const VKAuthRequireDialog: React.FC<{ connect?: () => void }> = ({
  connect,
}) => {
  return (
    <Dialog open>
      <DialogContent autoClose>
        <DialogHeader>
          ...
          <DialogDescription className="ms-space-y-2">
            ...
            <p>
              Также регистрация новых учётных записей доступна только через
              предварительную авторизацию через VK ID
            </p>
            <p>
              Для Подключения VK ID к Вашей учётной записи нажмите на кнопку
              ниже
            </p>
