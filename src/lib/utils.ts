    ...
    clsx([
      inputs[0]
        ...
        .map(val =>
          [
            ...val.split(":").slice(0, -1),
            val.split(":").slice(-1)[0].startsWith("ms-") ||
            val.split(":").slice(-1)[0].startsWith("-ms-")
              ? val.split(":").slice(-1)[0]
              : "ms-" + val.split(":").slice(-1)[0],
          ].join(":")
        ),
      ...inputs.splice(1),
    ])
...
export function dateFormat(date: Date, miniYear = false) {
  ...
  // date
  const year = miniYear
    ? date.getFullYear().toString().slice(2, 4).padStart(2, "0")
...
export function* range(startOrEnd: number, end?: number, step = 1) {
  let start = end ? startOrEnd : 0;
  let _end = end || startOrEnd;
  while (start < _end) {
    yield start;
    start += step;
  }
...
export function generateRangeArray(startOrEnd: number, end?: number, step = 1) {
  return Array.from(range(startOrEnd, end, step));
}
